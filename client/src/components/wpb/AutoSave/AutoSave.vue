...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toggleAutoSave() {
        this.patchUser(this.authUser._id, {'preferences.autoSave.value': this.autoSave});
      },
      manualSave() {
        if (!this.saved) {
          this.executeSave({pageId: this.page._id, interval: 0, manualSave: true});
        }
      },
      changeDebounce() {
        this.patchUser(this.authUser._id, {'preferences.autoSave.debounce': this.dbounce});
      },
    },
  };
</script>
