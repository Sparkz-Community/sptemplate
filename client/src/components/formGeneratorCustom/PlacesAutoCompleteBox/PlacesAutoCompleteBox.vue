  ...
  export default {
    ...
    computed: {
      attrs() {
        ...
        this.$lset(newVal, 'attrs.attrs.popup-content-style',
                   this.$lget(newVal, 'attrs.attrs.popup-content-style', ''));

        // div-attrs defaults
        this.$lset(newVal, 'div-attrs.class', this.$lget(newVal, 'div-attrs.class', 'col-12 col-sm-6'));

        // expansion-attrs defaults
        this.$lset(newVal, 'expansion-attrs.header-class',
                   this.$lget(newVal, 'expansion-attrs.header-class', 'bg-primary text-white'));
        this.$lset(newVal, 'expansion-attrs.expand-icon-class',
                   this.$lget(newVal, 'expansion-attrs.expand-icon-class', 'text-white'));
        this.$lset(newVal, 'expansion-attrs.style',
                   this.$lget(newVal, 'expansion-attrs.style', 'border-radius: 30px'));
        this.$lset(newVal, 'expansion-attrs.icon', this.$lget(newVal, 'expansion-attrs.icon', 'explore'));
        this.$lset(newVal, 'expansion-attrs.class',
                   this.$lget(newVal, 'expansion-attrs.class', 'shadow-1 overflow-hidden'));
