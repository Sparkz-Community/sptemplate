<template>
  <div class="q-pa-md">
    <div class="column q-my-sm">
      ...
      <icon-picker :model-value="$lget(element, 'attrs.name', '')"
                   @update:model-value="debounce({path: 'attrs.name', value: $event})"
                   path="attrs.name"
                   :q-field-attrs="{
                     label: '',
                     type: 'text',
    ...
    <div class="column" style="width: 50%">
      ...
      <div style="width: 90%; display: flex; flex-direction: row; align-items: center; background-color: #f2f2f2">
        <q-input style="width: 100%;"
               ...
               :model-value="separateValues($lget(element, 'attrs.size', ''))"
               @update:model-value="debounce({ path: 'attrs.size', 'model-value': $event.target['model-value'] +
                  pullUnit($lget(element, 'attrs.size', '36px'))})"/>
        <q-select style="font-size: 14px; border: 0; outline: 0; height: 25px;"
                :model-value="pullUnit($lget(element, 'attrs.size', ''))"
                @change="$emit('stylesUpdate', {
                  path: 'attrs.size',
                  'model-value': ['', '$unset', 'xs', 'sm', 'md', 'lg', 'xl'].includes($event.target['model-value']) ?
                      $event.target['model-value'] : separateValues($lget(element, 'attrs.size', '36px')) + $event.target.value
                }); disableCheck($event.target['model-value'])">
          <q-tooltip>
            Select Unit
          </q-tooltip>
          <option v-for="unit in fontSizeUnits" :key="unit" :value="unit">
            {{ unit }}
          </option>
        </q-select>
      </div>
    ...
    <div class="column q-my-sm">
      ...
      <q-input class="inputBackground"
             :model-value="$lget(element, 'attrs.color', '')"
             @update:model-value="debounce({path: 'attrs.color', value: $event.target['model-value']})"/>
    </div>
  </div>
</template>
