  ...
  export default {
        ...
        let basic = [
          {
            show: !this.hideSms,
            noLabel: this.noText,
            label: this.showText ? this.$lget(this.modelValue, this.smsPath) : 'Text',
            icon: 'mdi-message',
            href: `sms: ${this.$lget(this.modelValue, this.smsPath)}`,
          ...
          },
          {
            show: !this.hidePhone,
            noLabel: this.noText,
            label: this.showCall ? this.$lget(this.modelValue, this.phonePath) : 'Call',
            icon: 'mdi-phone',
            href: `tel: ${this.$lget(this.modelValue, this.phonePath)}`,
          ...
          },
          {
            show: !this.hideEmail,
            noLabel: this.noText,
            label: this.showEmail ? this.$lget(this.modelValue, this.emailPath) : 'Email',
            icon: 'mdi-email',
            href: `mailto: ${this.$lget(this.modelValue, this.emailPath)}`,
    ...
    methods: {
      requestVideoChat() {
        ...
        //       supportRequested: this.modelValue.name,
        //       host: this.$lget(this.host, '_id'),
        //     };
        //     new models.api.VideoRequests(chatRoom).create().then(() => {
        //       this.$successNotify('Request has been sent to ' + this.modelValue.name + '. We will notify you when they accept.');
        //     }).catch((err) => {
        //       console.log(err);
        //     });
        //   }).catch((err) => {
