  ...
  <div class="my-accounts">
    <div class="box-wrapper">
      <div v-for="(account, index) in accountList" :key="index" class="q-mb-lg">
        <account-profile-component :model-value="{account}" :hover="true"/>
      </div>
...
<script>
  ...
  import AccountFormDialogComponent from '@sparkz-community/common-client-lib/src/components/profile/accounts/AccountFormDialog';
  ...
  export default {
    ...
    computed: {
      auth() {
        return this.$useAuthStore();
      },
      accountList() {
        // return this.accounts.map(item => item.clone());
        return this.$lget(this.auth, 'accounts', []).map(item => item.clone());
      },
    },
    methods: {
      notifySaved(value) {
