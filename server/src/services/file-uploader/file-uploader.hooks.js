...
const handleUpload = async context => {
  ...
  try {
    ...
    console.error('src/utils/uploadMiddleware/utils/handleUpload.js', error);
    throw error;
  }
};
...
const handleDelete = async context => {
  ...
  try {
    ...
    const currentDoc = await coreCall(context, 'file-uploader').get(docId);
...
module.exports = {
  before: {
    all: [
      coreAuthentication({
        required: [
          {service: 'authentication', strategies: ['jwt', 'api-key']},
        ],
        optional: [],
      }),
      // beforeHook,
